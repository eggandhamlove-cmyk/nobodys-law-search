...
def toggles_from_markers(inner, labels):
    ...
    marker = re.compile(
        r'<p(?P<attrs>[^>]*)>\s*(?:<strong>)?(?P<label>' + alt + r')(?:</strong>)?\s*</p>',
        re.S,
    )
    ...
    for idx, m in enumerate(found):
        ...
        out.append(
            f'<details{id_attr} class="toggle item-toggle">'
            f'<summary><strong>{m.group("label")}</strong></summary>'
            f'<div class="toggle-body">{content}</div></details>'
        )
    return ''.join(out), len(found)


char_pat = re.compile(
    r'(<h3[^>]*>キャラクター作成詳細</h3>)(.*?)(?=<h3[^>]*>エリア詳細</h3>)',
    re.S,
)
char_count = 0
...
def char_repl(m):
    ...
    rebuilt, n = toggles_from_markers(
        inner,
...
# Pink quote line belongs to the quote heading only; following paragraph is normal body text.
s = re.sub(
    r'<blockquote(?P<attrs>[^>]*)>(?P<quote>.*?)</blockquote>\s*<p(?P<pattrs>[^>]*)>(?P<body>.*?)</p>',
    r'<div class="quote-group"><blockquote\g<attrs>>\g<quote></blockquote><p\g<pattrs>>\g<body></p></div>',
    s,
    flags=re.S,
)
