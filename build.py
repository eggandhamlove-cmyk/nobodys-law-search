...
def rt(xs):
    z=''
    for x in xs or []:
        t=html.escape(x.get('plain_text',''))
        a=x.get('annotations',{})
        ...
        if not href and x.get('type')=='mention':
            m=x.get('mention',{})
            if m.get('type')=='page' and m.get('page',{}).get('id'):
                href=notion_page_url(m['page']['id'])
        if href:
            t=f'<a href="{html.escape(href,quote=True)}" target="_blank" rel="noopener">{t}</a>'
        z+=t
    return z

def plain(xs):
    return ''.join(x.get('plain_text','') for x in xs or []).strip()

def heading_level(block):
    typ=block.get('type','')
    return int(typ[-1]) if typ in ('heading_1','heading_2','heading_3') else None
...
def render(bs):
    ...
    while i < len(bs):
        ...
        if typ=='bulleted_list_item':
            ...
            out.append(f'<li>{rt(d.get("rich_text"))}{sub}</li>'); i+=1; continue
        ...
        if typ in ('heading_1','heading_2','heading_3'):
            level=int(typ[-1]); title=rt(d.get('rich_text'))
            is_toggle=bool(d.get('is_toggleable'))
            nested=render(kids(b['id'])) if b.get('has_children') else ''
            # Some Notion exports/API responses expose toggle-heading contents as following siblings.
            if is_toggle and not nested:
                ...
                if collected:
                    nested=render(collected); i=j-1
            if is_toggle or nested:
                out.append(f'<details class="toggle heading-toggle"><summary><span class="h{level}">{title}</span></summary><div class="toggle-body">{nested}</div></details>')
            else:
            ...
            sub=render(kids(b['id'])) if b.get('has_children') else ''
            out.append(f'<details class="toggle"><summary>{rt(d.get("rich_text"))}</summary><div class="toggle-body">{sub}</div></details>')
        elif typ=='quote':
        ...
        elif typ=='table_of_contents':
            out.append('<nav class="toc"><b>目次</b><div class="toc-items"></div></nav>')
            ...
            target=d.get('page_id') or d.get('database_id') or ''
            out.append(f'<p><a href="{notion_page_url(target)}" target="_blank" rel="noopener">Notionページを開く</a></p>')
        elif typ=='callout':
            out.append(f'<aside>{rt(d.get("rich_text"))}</aside>')
        elif typ=='divider': out.append('<hr>')
        elif typ=='code':
            out.append(f'<pre><code>{html.escape("".join(x.get("plain_text","") for x in d.get("rich_text",[])))}</code></pre>')
        elif typ=='to_do':
            out.append(f'<label class="todo"><input type="checkbox" disabled{" checked" if d.get("checked") else ""}> {rt(d.get("rich_text"))}</label>')
        elif typ=='child_page':
            sub=render(kids(b['id']))
            out.append(f'<section><h2>{html.escape(d.get("title",""))}</h2>{sub}</section>')
        elif typ=='table':
            out.append('<div class="table"><table>')
            for row in kids(b['id']):
                out.append('<tr>'+''.join(f'<td>{rt(c)}</td>' for c in row.get('table_row',{}).get('cells',[]))+'</tr>')
            out.append('</table></div>')
        elif typ=='synced_block':
            out.append(render(kids(b['id'])) if b.get('has_children') else '')
        elif typ in ('column_list','column'):
            out.append(f'<div class="{typ}">{render(kids(b["id"])) if b.get("has_children") else ""}</div>')
        else:
            x=rt(d.get('rich_text')); sub=render(kids(b['id'])) if b.get('has_children') else ''
            if x: out.append(f'<p>{x}</p>')
            out.append(sub)
...
for v in page.get('properties',{}).values():
    if v.get('type')=='title':
        title=''.join(x.get('plain_text','') for x in v.get('title',[])) or title; break
