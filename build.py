...
def block_anchor(block_id): return 'block-'+block_id.replace('-','')
...
def localize_href(href):
    ...
    frag=(u.fragment or '').replace('-','')
    if ('notion.so' in host or 'notion.com' in host) and len(frag)==32 and all(c in '0123456789abcdefABCDEF' for c in frag):
        return '#block-'+frag.lower()
    return href
...
def rt(xs):
    ...
    for x in xs or []:
        ...
        href=localize_href(href)
        if href:
            attrs='' if href.startswith('#') else ' target="_blank" rel="noopener"'
            t=f'<a href="{html.escape(href,quote=True)}"{attrs}>{t}</a>'
        z+=t
    return z
...
def render(bs):
    ...
    while i<len(bs):
        b=bs[i]; typ=b['type']; d=b.get(typ,{}); bid=block_anchor(b['id'])
        if typ=='bulleted_list_item':
            if ol:out.append('</ol>');ol=False
            if not ul:out.append('<ul>');ul=True
            sub=render(kids(b['id'])) if b.get('has_children') else '';out.append(f'<li id="{bid}">{rt(d.get("rich_text"))}{sub}</li>');i+=1;continue
        ...
        if typ in ('heading_1','heading_2','heading_3'):
            ...
            if is_toggle or nested:out.append(f'<details id="{bid}" class="toggle heading-toggle"><summary><span class="h{level}">{title}</span></summary><div class="toggle-body">{nested}</div></details>')
            else:out.append(f'<h{level} id="{bid}">{title}</h{level}>')
        elif typ=='paragraph':
            x=rt(d.get('rich_text'));sub=render(kids(b['id'])) if b.get('has_children') else ''
            if x:out.append(f'<p id="{bid}">{x}</p>')
            elif sub:out.append(f'<div id="{bid}">{sub}</div>');sub=''
            out.append(sub)
        elif typ=='toggle':
            sub=render(kids(b['id'])) if b.get('has_children') else '';out.append(f'<details id="{bid}" class="toggle"><summary>{rt(d.get("rich_text"))}</summary><div class="toggle-body">{sub}</div></details>')
        elif typ=='quote':
            text=plain(d.get('rich_text'))
            if text=='目次':out.append(f'<nav id="{bid}" class="toc"><b>目次</b><div class="toc-items"></div></nav>')
            else:out.append(f'<blockquote id="{bid}">{rt(d.get("rich_text"))}</blockquote>')
        elif typ=='table_of_contents':out.append(f'<nav id="{bid}" class="toc"><b>目次</b><div class="toc-items"></div></nav>')
        elif typ in ('bookmark','link_preview','embed'):
            u=d.get('url','');out.append(f'<p id="{bid}" class="linkcard"><a href="{html.escape(u,quote=True)}" target="_blank" rel="noopener">{html.escape(u)}</a></p>')
        elif typ=='link_to_page':
            target=d.get('page_id') or d.get('database_id') or '';out.append(f'<p id="{bid}"><a href="{notion_page_url(target)}" target="_blank" rel="noopener">Notionページを開く</a></p>')
        elif typ=='callout':
            sub=render(kids(b['id'])) if b.get('has_children') else ''
            out.append(f'<aside id="{bid}">{rt(d.get("rich_text"))}{sub}</aside>')
        elif typ=='divider':out.append(f'<hr id="{bid}">')
        elif typ=='code':out.append(f'<pre id="{bid}"><code>{html.escape("".join(x.get("plain_text","") for x in d.get("rich_text",[])))}</code></pre>')
        elif typ=='to_do':out.append(f'<label id="{bid}" class="todo"><input type="checkbox" disabled{" checked" if d.get("checked") else ""}> {rt(d.get("rich_text"))}</label>')
        elif typ=='child_page':out.append(f'<section id="{bid}"><h2>{html.escape(d.get("title",""))}</h2>{render(kids(b["id"]))}</section>')
        elif typ=='table':
            out.append(f'<div id="{bid}" class="table"><table>')
            for row in kids(b['id']):out.append('<tr>'+''.join(f'<td>{rt(c)}</td>' for c in row.get('table_row',{}).get('cells',[]))+'</tr>')
            out.append('</table></div>')
        elif typ=='synced_block':out.append(f'<div id="{bid}">'+(render(kids(b['id'])) if b.get('has_children') else '')+'</div>')
        elif typ in ('column_list','column'):out.append(f'<div id="{bid}" class="{typ}">{render(kids(b["id"])) if b.get("has_children") else ""}</div>')
        else:
            x=rt(d.get('rich_text'));sub=render(kids(b['id'])) if b.get('has_children') else ''
            if x:out.append(f'<p id="{bid}">{x}</p>')
            elif sub:out.append(f'<div id="{bid}">{sub}</div>');sub=''
